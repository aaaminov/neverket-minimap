...
На Minecraft 1.21.1 текущая яркость берётся из version-specific `ClientLevel.getSkyDarken(partialTick)`, затем применяется к CPU-копии ARGB-пикселей и загружается только при изменении tint. Старый GUI rendering API получает уже готовую дневную или затемнённую текстуру. Фон полноэкранного экрана затемняет весь мир до отрисовки карты, поэтому при нулевой заливке неизведанного яркий мир не просвечивает. Миникарта рисуется перед vanilla HUD, поэтому F3, чат, scoreboard, эффекты и другие HUD-слои остаются поверх неё. Высота, тип поверхности и цвет каждого блока получаются одним общим проходом чанка; отдельных проходов для цвета и полуразведанного рельефа нет.

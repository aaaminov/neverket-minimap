...
Другие игроки отмечаются бирюзовым крестом на мини-карте и полноэкранной карте. Мод не запрашивает их координаты: показываются только уже известные client world игроки, обычно в пределах server tracking distance. Показ можно отключить в настройках.
...
- Независимая заливка неизведанной территории мини-карты и полной карты от 0 до 100%.
...
Опция показывает вокруг игрока упрощённый чёрно-белый контур загруженной поверхности. Вода и суша имеют разные оттенки, а внешний край плавно исчезает через прозрачность без примеси тёмного RGB. Палитра, alpha-композиция границ и коэффициенты рельефа одинаковы на Minecraft 1.21.1 и 26.2. Контуры собираются тем же общим проходом чанка, что и цвет, и работают в обоих режимах записи.
...
Стандартный preset: размер 112 пикселей, масштаб 8 блоков на пиксель, непрозрачность 100%, вращение выключено, указатель севера подготовлен к показу при включении вращения, стороны света скрыты, чёрная рамка, контуры местности включены с радиусом 16 чанков, ночное затемнение 100%, быстрая метка `Жёлтый крест`, на рамке до 6 баннеров. Режим записи остаётся `Сделанные карты` и не зависит от этого preset.
...
На Minecraft 1.21.1 текущая яркость берётся из version-specific `ClientLevel.getSkyDarken(partialTick)`, затем применяется к CPU-копии ARGB-пикселей и загружается только при изменении tint. Старый `GuiGraphics.blit(ResourceLocation, …)` не включает blending самостоятельно, поэтому адаптер явно включает его на время отрисовки текстуры: промежуточные значения alpha работают так же, как в 26.2. Фон полноэкранного экрана затемняет весь мир один раз до отрисовки карты; повторный вызов `renderBackground()` из `Screen.render()` подавляется, поэтому overlay не накладывается поверх карты. При нулевой заливке неизведанного яркий мир не просвечивает, а непрозрачные пиксели карты не затемняются фоном. Миникарта рисуется перед vanilla HUD, поэтому F3, чат, scoreboard, эффекты и другие HUD-слои остаются поверх неё. Высота, тип поверхности и цвет каждого блока получаются одним общим проходом чанка; отдельных проходов для цвета и полуразведанного рельефа нет.
...
- координаты;
- стороны света;
- вращение вслед за направлением взгляда игрока;
- красно-белая компасная стрелка севера на рамке вращающейся мини-карты;
- показ и цвет рамки.

### Вид карты и туман местности

- заливка неизведанной территории мини-карты от 0 до 100% с шагом 5%;
- отдельная заливка неизведанной территории полной карты от 0 до 100% с шагом 5%;
...
Сканирование выполняется только на клиенте и только для уже загруженных чанков. Один проход `16×16` одновременно получает высоту, тип поверхности и vanilla `MapColor`; повторного прохода ради тумана нет. За тик обновляются не более двух чанков в пределах общего планируемого бюджета 1 мс.

Приоритеты планировщика:

1. Новый текущий чанк.
2. Повторная проверка чанка игрока примерно раз в секунду.
3. Повторная проверка восьми соседних чанков по кругу; полный круг занимает около четырёх секунд.
4. Первичная запись ещё неизвестных загруженных чанков от центра к краю.
5. Редкая повторная проверка дальних чанков.

При переходе игрока в новый чанк единая очередь сразу переносится к новой позиции и снова идёт от центра к краю. За один тик она может быстро пропустить до 128 уже известных или незагруженных чанков, но не начинает новую тяжёлую обработку после истечения бюджета.
...
Мини-карта принимает новые данные не чаще одного раза в 250 мс, но её уже готовая текстура, метки, игроки и указатель севера поворачиваются плавно каждый кадр по интерполированному направлению взгляда. Для квадратной формы текстура хранит запас до полной диагонали, поэтому при вращении не появляются пустые углы. Для более дорогой полноэкранной текстуры интервал составляет 350 мс.

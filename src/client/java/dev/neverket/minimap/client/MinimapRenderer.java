...
public final class MinimapRenderer implements AutoCloseable {
	...
	private void drawMapTexture(GuiGraphics graphics, int x, int y, int size, int tint, double rotationDegrees) {
		...
		try {
			graphics.pose().translate(x + size / 2.0, y + size / 2.0, 0.0);
			graphics.pose().mulPose(Axis.ZP.rotationDegrees((float)rotationDegrees));
			graphics.pose().translate(-(x + size / 2.0), -(y + size / 2.0), 0.0);
			if (circular) {
				this.viewTexture.blitCircular(graphics, x, y, size, size, tint);
			} else {
	...
	private void drawCardinalDirections(GuiGraphics graphics, int x, int y, int size, double rotationDegrees) {
		boolean circular = this.config.shape == ModConfig.Shape.CIRCLE;
	...
	private static void drawNorthIndicator(
		GuiGraphics graphics, int x, int y, int size, boolean circular, double rotationDegrees
	) {
	...
	private static void drawCompassNeedle(GuiGraphics graphics, int centerX, int centerY, double directionX, double directionY) {
	...
	private static void drawNeedleOutline(
		GuiGraphics graphics, int centerX, int centerY, double directionX, double directionY,

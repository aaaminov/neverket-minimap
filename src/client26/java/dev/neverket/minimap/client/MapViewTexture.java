...
public final class MapViewTexture implements AutoCloseable {
	...
	public MapViewTexture(Minecraft minecraft, GuiGraphicsAccess graphicsAccess, Identifier id, int viewWidth, int viewHeight) {
		this(minecraft, graphicsAccess, id, viewWidth, viewHeight, 0, false);
	}

	public MapViewTexture(
		Minecraft minecraft,
		GuiGraphicsAccess graphicsAccess,
		Identifier id,
		int viewWidth,
		int viewHeight,
		int centerSnapPixels
	) {
		this(minecraft, graphicsAccess, id, viewWidth, viewHeight, centerSnapPixels, false);
	}
	...
	public void blitOverscanned(GuiGraphicsExtractor graphics, int x, int y, int width, int height, int color) {
		this.ensureCreated();
		...
		graphics.blit(
			RenderPipelines.GUI_TEXTURED, this.id,
			x - drawPaddingX, y - drawPaddingY,
			this.sourceU - this.rotationPaddingX, this.sourceV - this.rotationPaddingY,
			width + drawPaddingX * 2, height + drawPaddingY * 2,
			this.viewWidth + this.rotationPaddingX * 2, this.viewHeight + this.rotationPaddingY * 2,
			this.textureWidth, this.textureHeight, color
		);
	}
	...
	public void blitCircular(GuiGraphicsExtractor graphics, int x, int y, int width, int height, int color) {
		...
		this.graphicsAccess.submit(graphics, new CircularBlitRenderState(
			...
			this.textureWidth, this.textureHeight, color, this.graphicsAccess.peekScissor(graphics)
		));
	}
		...
		for (int y = 1; y < this.textureHeight - 1; y++) {
			for (int x = 1; x < this.textureWidth - 1; x++) {
				...
				if (differentTerrain(kind, kinds[index - 1]) || differentTerrain(kind, kinds[index + 1])
					...
					this.texture.getPixels().setPixel(x, y, TerrainFogStyle.composeBoundary(unknownColor, fade));
				}
			}
		}
	}
		...
		for (int y = 0; y < this.textureHeight - 1; y++) {
			for (int x = 0; x < this.textureWidth - 1; x++) {
				...
				if ((right != NO_HEIGHT && kinds[index + 1] == kinds[index] && Math.floorDiv(right, 16) != band)
					...
					this.texture.getPixels().setPixel(x, y, TerrainFogStyle.applyContour(base, fade));
				}
			}
		}
	}
